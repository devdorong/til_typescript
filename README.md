...
- 함수에서는 `매개변수` name 에 대해 `타입추론을 안해줌.`
- 일반 변수는 타입추론을 지원해줌.
...
- 선택적으로 활용할 수 있다.
- 옵셔널을 활용하면 `undefined 를 허용`하겠다.
- 옵셔널 매개변수는 반드시 필수 매개변수 뒤에 위치한다.
...
```ts
function showName(name: string, contry: string = "한국") {
  console.log(name);
  console.log(contry); // 한국
}
...
showMember("아이유", "지민"); // 2개가 가능하므로 오류
```
...
- 어떤 변수, 어떤 반환값이 `특정 타입인지 확인하는 용도의 함수`
