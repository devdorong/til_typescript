...
// 코드 중에 값을 변경 하겠다.
// d = "고양이"; // Error 타입 오류 발생
// 아래는 가능합니다.
d = { name: "고양이", bark: () => "고양이 야옹~" }; // Error 타입 오류 발생
```
...
// 타입을 체크해주는 함수 만들기
const ori: any = new Dog("오리", 5, "청동오리");
// 타입을 체크해서 맞다면 실행하자.
function instanceOfDog(who: any) {
  return "dance" in who;
}
if (ori) {
  ori;
...
// Dog 는 필수 속성인 breez 가 필요하지만 Animal 에는 없다.
