...
- 비동기 처리는 시간이 많이 걸리는 작업 진행 중에 다른 일도 병렬로 처리하도록 함.
...
- `Query` 라는 단어도 알고 계셔야 합니다. (질의문, Request 한 결과물)

### 4.1. 쿼리의 이해

- `https://search.naver.com/search.naver?where=nexearch&sm=top_hty&fbm=0&ie=utf8&query=iu&ackey=fu7sz5z4`
- `https://isearch.interpark.com/result?q=부산`
- 도메인 : [https://isearch.interpark.com](https://isearch.interpark.com/)
- 라우터 경로 : /result
- 쿼리(자료요청 문자열)의 시작 : ?
- 실제쿼리 : `q=부산&referrer=`
...
- `REST Api` 칭함(주소/라우터)
- `CRUD` 작업 (DB 를 CReate, Read, Updatem Delete)
      ...
      function getPosts() {
        ...
        xhr.onload = function () {
          ...
          if (xhr.status === 200) {
            ...
            // 90% 이상은 프론트에서 잘못된 ㅊ요청을 한 경우가 대부분
            console.log("없는 페이지로 접속하였습니다.");
      ...
      function getPhotos() {
        ...
        xhr.onload = function () {
          if (xhr.status === 200) {
            ...
            console.log("주소 확인하세요.");
          } else if (Xhr.status === 505) {
            console.log("잠시후에 시도해주세요.");
          }
        };
      }
      //   getPhotos();
...
function getData(addr: string, method: Method): Promise<string> {
  ...
  return new Promise(function (resolve: Resolve, rejected: Rejected) {
    // 하고 싶은 XHR
    const xhr: XMLHttpRequest = new XMLHttpRequest();
    xhr.open(method, url);
    xhr.send();
...
- 반드시 function 안쪽에 try ~ catch 를 적어줌.
