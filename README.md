...
// 위의 변수는 string 으로 판정이 되므로 아래 문장에서 오류가 발생함.
...
nullOrString = Math.random() > 0.5 ? null : ["아이유"];
// if 로 참인 것을 거르고 처리
if (nullOrString) {
  // 글자배열
...
- new 클래스명() 으로 만들어진 변수를 인스턴스 변수라고 합니다.
